...
import { ClientPedidoInfoCard } from "./ClientPedidoInfoCard"

interface ClientPedidoDetailPageProps {
  id: string
}

export function ClientPedidoDetailPage({ id }: ClientPedidoDetailPageProps) {
  const router = useRouter()
  const { data: pedido, isLoading, isError, refetch } = useClientPedidoDetail(id)
  ...
  if (isError || !pedido) {
    return (
      <div className="flex flex-col items-center justify-center gap-4 w-full max-w-3xl mx-auto py-24 text-center">
        <p className="text-sm font-medium text-destructive">Error al cargar el pedido</p>
    ...
    <div className="space-y-6 w-full max-w-3xl mx-auto px-2 py-2 md:px-4">
      <div className="space-y-1">
        ...
        <h1 className="text-2xl font-bold tracking-tight">
          Pedido — {pedido.nombre_restaurante}
        </h1>
      ...
      <ClientPedidoInfoCard pedido={pedido} />
    </div>
  )
}
